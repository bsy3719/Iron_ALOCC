...
class ALOCC_Model():
    ...
    def build_generator(self, input_shape):
        ...
        # Encoder.
        x = Conv2D(filters=self.df_dim , kernel_size = 5, strides=2, padding='same')(image)
        x = BatchNormalization()(x)
        x = LeakyReLU()(x)
        x = Conv2D(filters=self.df_dim * 2, kernel_size = 5, strides=2, padding='same')(x)
        x = BatchNormalization()(x)
        x = LeakyReLU()(x)
        x = Conv2D(filters=self.df_dim * 4, kernel_size = 5, strides=2, padding='same')(x)
        x = BatchNormalization()(x)
        x = LeakyReLU()(x)

        x = Conv2D(filters=self.df_dim * 8, kernel_size=5, strides=2, padding='same')(x)
        x = BatchNormalization()(x)
        x = LeakyReLU()(x)

        # Decoder.
        # TODO: need a flexable solution to select output_padding and padding.
        x = Conv2DTranspose(self.gf_dim * 4, kernel_size=5, strides=2, activation='relu', padding='same', output_padding=1)(x)
        x = BatchNormalization()(x)

        x = Conv2DTranspose(self.gf_dim*2, kernel_size = 5, strides=2, activation='relu', padding='same', output_padding=1)(x)
        x = BatchNormalization()(x)
        x = Conv2DTranspose(self.gf_dim*1, kernel_size = 5, strides=2, activation='relu', padding='same', output_padding=1)(x)
        x = BatchNormalization()(x)
        x = Conv2DTranspose(self.c_dim,    kernel_size = 5, strides=2, activation='sigmoid', padding='same', output_padding=1)(x)

        # x = Conv2D(self.gf_dim*1, kernel_size=5, activation='relu', padding='same', name='g_decoder_h0_conv')(x)
        # x = UpSampling2D((2, 2))(x)
        # x = Conv2D(self.gf_dim*1, kernel_size=5, activation='relu', padding='same', name='g_decoder_h1_conv')(x)
        # x = UpSampling2D((2, 2))(x)
        # x = Conv2D(self.gf_dim*2, kernel_size=3, activation='relu', padding='same', name='g_decoder_h2_conv')(x)
        # x = UpSampling2D((2, 2))(x)
        # x = Conv2D(self.c_dim, kernel_size=5, activation='sigmoid', padding='same', name='g_decoder_h3_conv')(x)
    ...
    def train(self, epochs, batch_size = 128, sample_interval=500):
        ...
        for epoch in range(epochs):
            ...
            for idx in range(0, batch_idxs):
                ...
                #self.adversarial_model.train_on_batch(batch_clean_images, [batch_clean_images, ones])
                #g_loss = self.adversarial_model.train_on_batch(batch_clean_images, [batch_clean_images, ones])
        ...
        plt.grid()
        #plt.plot(plot_epochs, plot_d_losses)
    ...
    def save(self, step):
        ...
        #self.adversarial_model.save(model_name)


    def load_dir(self, batch_size):
        train_datagen = ImageDataGenerator(rescale=1. / 255)

        for e in range(1):
            print('Epoch', e)
            batches = 0
            total = np.ndarray(shape=(0, 64, 64, 3), dtype=np.float32)
            for x_batch, y_batch in train_datagen.flow_from_directory('hwang/train', target_size=(64, 64),  batch_size=batch_size):
                print(x_batch.shape)
                print(type(x_batch))
                total = np.vstack((total, x_batch))
                batches += 1
                if batches >= 2000 / batch_size:
                    print(batches)
                    break
...
if __name__ == '__main__':
    model = ALOCC_Model(dataset_name='mnist', input_height=64, input_width=64)
    model.train(epochs=30, batch_size=128, sample_interval=500)
